...
class ObjectZod<T extends Record<string, AbstractValidationBase<any>>> extends YelixValidationBase<T> {
  input: YelixInput;
  subFields: Mutable<T>;
  ...
  private setSubField<K extends keyof T>(key: K, value: YelixValidationBase<any>) {
  ...
  private loadKeys(obj?: T) {
    if (obj) {
      Object.keys(obj).forEach((key) => {
        this.setSubField(key as keyof T, obj[key] as unknown as YelixValidationBase<any>);
      });
    }
  }
  ...
  override validate(value: any, config?: ValidateConfig): ValidateResult {
    ...
    for (const [key, validator] of Object.entries(this.subFields)) {
      ...
      if (!fieldResult.isOk) {
        fieldResult.errors.forEach((error: { message: string; key: string; from: string; }) => {
          subFieldErrors.push({
